  ...
  <div class="container" style="width:99%;margin-top:-25px;">
	...
	<div class="toolbar" style="float:left;padding-top:10px;padding-left:15px;">
		<el-form :inline="true" :model="filters" :size="size">
			...
			<el-form-item>
				<kt-button :label="$t('action.search')" perms="sys:wmMonthwaterout:view" type="primary" @click="findPage(null)"/>
			</el-form-item>
			<el-form-item>
				<kt-button :label="$t('action.add')" perms="sys:wmMonthwaterout:add" type="primary" @click="handleAdd" />
			</el-form-item>
		</el-form>
	</div>
	<!--表格内容栏-->
	<kt-table permsEdit="sys:wmMonthwaterout:edit" permsDelete="sys:wmMonthwaterout:delete"
	...
	<el-dialog :title="operation?'新增':'编辑'" width="40%" :visible.sync="editDialogVisible" :close-on-click-modal="false">
		<el-form :model="dataForm" label-width="80px" :rules="dataFormRules" ref="dataForm" :size="size">
			...
			<el-form-item label="出水量" prop="waterout"  v-if="dataForm.isPrimaryKey">
				<el-input v-model="dataForm.waterout" auto-complete="off"></el-input>
			</el-form-item>
...
export default {
	...
	data() {
		return {
			...
			columns: [
				...
				{prop:"waterout", label:"出水量", minWidth:100},
	...
	methods: {
		...
		findPage: function (data) {
			...
			this.$api.wmMonthwaterout.findPage(this.pageRequest).then((res) => {
				this.pageResult = res.data
			}).then(data!=null?data.callback:'')
		},
		// 批量删除
		handleDelete: function (data) {
			this.$api.wmMonthwaterout.batchDelete(data.params).then(data!=null?data.callback:'')
		},
		// 显示新增界面
		handleAdd: function () {
			...
			this.$refs.dataForm.validate((valid) => {
				if (valid) {
					this.$confirm('确认提交吗？', '提示', {}).then(() => {
						...
						this.$api.wmMonthwaterout.save(params).then((res) => {
							if(res.code == 200) {
